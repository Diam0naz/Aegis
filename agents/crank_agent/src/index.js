...
// ── PDA Helpers ───────────────────────────────────────────────────
function getMarketPDA(authority, questionHash, programId) {
    return PublicKey.findProgramAddressSync([Buffer.from("market"), authority.toBuffer(), questionHash], programId)[0];
}
function getLpPoolPDA(market, programId) {
    return PublicKey.findProgramAddressSync([Buffer.from("lp_pool"), market.toBuffer()], programId)[0];
}
...
export class CrankAgent {
    ...
    async buildRemainingAccounts(orders, yesMint, noMint) {
        ...
        for (const { pubkey: orderPDA, order } of orders) {
            ...
            const userTokenAccount = getAssociatedTokenAddressSync(mint, order.user, false);
            // Verify the token account exists — skip if not initialised
            const info = await this.connection.getAccountInfo(userTokenAccount);
            if (!info) {
                console.warn(`  ⚠ Skipping order ${orderPDA.toBase58()} — token account not initialised`);
                continue;
            }
            // Interleaved: [order_pda, user_token_account]
            remainingAccounts.push({ pubkey: orderPDA, isSigner: false, isWritable: true }, { pubkey: userTokenAccount, isSigner: false, isWritable: true });
        }
        return remainingAccounts;
    }
    async trySettle(marketPubkey, market, currentSlot) {
        const key = marketPubkey.toBase58();
        if (this.settling.has(key))
            return;
    ...
    async getCollateralMint(market) {
        const vaultInfo = await this.connection.getParsedAccountInfo(market.collateralVault);
        const parsed = vaultInfo.value?.data?.parsed;
        const mint = parsed?.info?.mint;
        if (!mint)
            throw new Error(`Could not parse mint from vault ${market.collateralVault}`);
...
async function registerIdlWithSurfpool(connection, idl, programId) {
    try {
