...
import { computePoolMetrics } from "./pdl-calculator.js";
// ── Load environment ─────────────────────────────────────────────────
...
export class LpManagementAgent {
    ...
    cycleCount = 0;
    constructor(connection, program, wallet, kpiTargets, alertThresholds) {
    ...
    async start() {
        ...
        const marketsFile = process.env["MARKETS_FILE"] || "../crank_agent/markets.json";
    ...
    async monitorCycle() {
        ...
        for (const addrStr of this.knownMarkets) {
            try {
                ...
                activeMarkets++;
                // Derive LP pool PDA
                const [lpPoolPDA] = PublicKey.findProgramAddressSync([Buffer.from("lp_pool"), pubkey.toBuffer()], this.program.programId);
                let lpPool;
                try {
                    lpPool = await this.program.account.lpPool.fetch(lpPoolPDA);
                }
                catch {
                ...
                const poolAgeSlots = BigInt(Math.max(0, currentSlot - Number(lpPool.lastSettledSlot)));
                const metrics = computePoolMetrics(addrStr, bParam, yesQty, noQty, totalLiquidity, totalLpSupply, cumulativeFees, SLOT_DURATION_SECS, poolAgeSlots);
                ...
                const onChainPdlBps = this.deriveOnChainPdlBps(totalLiquidity, cumulativeFees, bParam, yesQty, noQty);
                ...
                const pdlIndicator = metrics.pdlBps >= 0 ? "🟢" : metrics.pdlBps > -500 ? "🟡" : "🔴";
                if (this.cycleCount % 10 === 1) {
                    // Detailed log every 10th cycle
                    console.log(`  ${pdlIndicator} Market ${marketId}... | ` +
    ...
    deriveOnChainPdlBps(totalLiquidity, cumulativeFees, bParam, yesQty, noQty) {
        ...
        const metrics = computePoolMetrics("verification", bParam, yesQty, noQty, totalLiquidity, totalLiquidity, // assume 1:1 LP supply for verification
        cumulativeFees, SLOT_DURATION_SECS, 1n);
...
async function main() {
    ...
    const kpiTargets = {
        ...
        falsePositiveMaxPct: Number(process.env["KPI_FALSE_POSITIVE_MAX_PCT"]) || 5.0,
        minPoolsMonitored: Number(process.env["KPI_MIN_POOLS_MONITORED"]) || 20,
    };
    ...
    const agent = new LpManagementAgent(connection, program, wallet, kpiTargets, alertThresholds);
    // Load known markets
